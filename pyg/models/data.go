...
type TpshopCategory struct {//首页三级联动表
	Id         int
...
func init() {
	...
	orm.RegisterDataBase("default", "mysql", "root:123456@tcp(192.168.11.112:3306)/pyg")

	orm.RegisterModel(new(User), new(Address),new(TpshopCategory),new(Goods),new(GoodsImage),new(GoodsSKU),new(GoodsType),new(IndexGoodsBanner),new(IndexPromotionBanner),new(IndexTypeGoodsBanner),new(OrderInfo),new(OrderGoods))
